...
#Print table
table1::table1(~ Sex+AgeAtDOI+YEARDOI+Cause+AIS+NLI_level, data = emsci.trauma.sex.va.a1)
...
#Calculate the change in age distribution over time and create table with model output
age_model <-lm(AgeAtDOI~YEARDOI, data=emsci.trauma.sex.va.a1)
summary(age_model)
...
#---Visualization: Change in age distribution over 20 years
...
ggplot(
  ...
  labs(title = 'Age at Injury between 2001 and 2019') +facet_grid(.~emsci.trauma.sex.va.a1$Sex)+
...
#----Plot population pyramide for year and color by Sex ----

#Calculate the number of patients per year by sex
...
model1<- lm(as.numeric(TMS)~YEARDOI*ExamStage_weeks+AIS+NLI_level+Sex+AgeAtDOI, data=emsci.ais)
summary(model1)
...
tms.lm<-subset(emsci.ais, YEARDOI=="2015") %>%
  group_by(AIS) %>%         
  do(tidy(lmer(as.numeric(TMS)~ExamStage_weeks+(1|Patientennummer), data=.))%>%
       mutate(
         'CI low' = estimate - std.error,
         'CI high' = estimate + std.error
       )
