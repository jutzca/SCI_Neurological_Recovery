...
for (j in rescaled.nli){
  for (i in ais.score){
    ...
    if (nrow(df1) == 0) next
    mixed.lmer<- nlme::lme(WISCI ~ ExamStage_weeks.rescaled+AgeAtDOI.rescaled+Cause_new, df1, random = ~ 1|Patientennummer, na.action = na.exclude)
    ...
    Cause_new.tval <- coef(summary(mixed.lmer))[16]
    ...
    Cause_new.pval <- coef(summary(mixed.lmer))[20]
    ...
    df <- data.frame( plegia= j,AIS = i, intercept.estimate = cfit[1], time_rescaled.estimate = cfit[2],
                      age.estimate= cfit[3], Cause_new.estimate= cfit[4],intercept.std= cfit[5],
                      time_rescaled.std =cfit[6],
                      age.std =cfit[7],
                      Cause_new.std =cfit[8],
                      intercept.df =cfit[9],
                      time_rescaled.df =cfit[10],
                      age.df =cfit[11],
                      Cause_new.df =cfit[12],
                      intercept.tval =cfit[13],
                      time_rescaled.tval =cfit[14],
                      age.tval =cfit[15],
                      Cause_new.tval =cfit[16],
                      intercept.pval =cfit[17],
                      time_rescaled.pval =cfit[18],
                      age.pval =cfit[19],
                      Cause_new.pval =cfit[20],
                      stringsAsFactors = F)
...
write.csv(new_data_4[,c(13,3:9,12)],"/Users/jutzca/Documents/Github/SCI_Neurological_Recovery/EMSCI/Tables/Sensitivity.Analysis_ischemic-vs-traumatic/emsci.mixed.models.results_WISCI_cause_of_injury.csv", row.names = F)
